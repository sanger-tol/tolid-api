    ...
    *data_sources: DataSource,
    ...
    def create_request():
        ...
        for row in request.json:
            species_id = row.get('speciesTaxonomyId')
            ...
            if existing_requests_count > 0:
                return {'detail': f'Request already exists for {species_id}-{specimen_id}'}, 400
            ...
            if existing_tolids_count > 0:
                return {'detail': f'ToLID already exists for {species_id}-{specimen_id}'}, 400
            ...
            requests_to_upsert.append(
                session.data_object_factory(
                    ...
                    attributes={
                        'species_id': species_id,
                        'requested_species_id': row.get('requestedTaxonomyId'),
                        'specimen_id': specimen_id,
                        'confirmation_name': row.get('speciesName'),
                        'status': 'Pre-pending'
                    },
