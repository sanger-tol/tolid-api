...
from main.model import TolidSpecies, TolidSpecimen, db
    ...
    for row in sheet.iter_rows(min_row=current_row,
        ...
        if (assign):
            ...
            if (re.search(r'sp\.$', scientific_name)):
                errors.append({'message': f'Row {current_row}'
                              + f': Genus only for {scientific_name}'
                              + ', not assigning ToLID'})
                ok = False
            else:
                ...
                if existing_species is None:
                    ...
                    if (scientific_name != existing_species.name):
                        errors.append({'message': f'Row {current_row}'
                                       f': Expecting {scientific_name}'
                                       f', got {existing_species.name}'})
                        ok = False
                    else:
